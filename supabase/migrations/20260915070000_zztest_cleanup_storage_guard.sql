-- cleanup_zztest_data : la bascule session_replication_role n'est plus systématique.
-- Le rôle propriétaire n'a plus le droit de la poser, ce qui faisait échouer tout le
-- nettoyage même quand aucun fichier ZZTEST n'était à supprimer (cas le plus courant).
-- Désormais : bascule seulement s'il y a des objets de stockage, et message explicite
-- demandant le passage par l'API Storage si le droit manque.
...
COMMENT ON FUNCTION public.cleanup_zztest_data(boolean) IS
  'Supprime les jeux ZZTEST / @example.invalid et les phrases test_phrases ZZTEST. dry_run=true par défaut. Admin uniquement. La bascule session_replication_role n''a lieu que s''il existe des fichiers ZZTEST.';
