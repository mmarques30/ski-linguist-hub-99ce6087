...
--
-- Tous les jobs sont (re)créés INACTIFS. L'activation se fait modèle par
-- modèle depuis /admin/emails après validation du texte correspondant.
...
INSERT INTO public.edge_dispatch_config (key, value, description) VALUES
  ...
  ('vault_secret_name',
   ...
   'Nom du secret Vault contenant la clé service_role. Absent = appel sans en-tête Authorization.')
...
-- ---------------------------------------------------------------------------
-- 5. (Re)création des jobs — tous inactifs
-- ---------------------------------------------------------------------------
...
BEGIN
  FOR spec IN
    SELECT * FROM (VALUES
      ...
      ('generate-monthly-charges',   '0 2 1 * *',  'generate-monthly-charges')
...
SELECT
  ...
  jsonb_build_object(
    ...
    'secret_vault_pose', EXISTS (
      SELECT 1 FROM public.edge_dispatch_config WHERE key = 'vault_secret_name'
    )
  );
