-- cleanup_zztest_data : autoriser l'appel depuis l'edge (clé service).
-- L'edge a déjà vérifié has_role(admin) avant l'RPC. Sans cela, is_admin()
-- échoue (auth.uid() NULL) et le front affiche « Erreur ».
...
DECLARE
  ...
  result jsonb;
BEGIN
  -- Autorise aussi le rôle service_role : l'edge cleanup-zztest vérifie
  -- has_role(admin) puis appelle l'RPC avec la clé service (auth.uid() NULL).
  IF NOT (
    public.is_admin()
    OR coalesce(auth.role(), '') = 'service_role'
  ) THEN
    RAISE EXCEPTION 'Réservé à un compte administrateur ou à la fonction Edge cleanup-zztest'
      USING ERRCODE = '42501';
  END IF;
