-- cleanup_zztest_data : le dry-run dit maintenant QUI a créé chaque stagiaire ZZTEST.
--
-- Incident du 15/09/2026 : un stagiaire « ZZTEST Camille » et son inscription
-- FLI-260006 ont été supprimés comme reliquat d'une démonstration alors qu'ils
-- venaient d'être saisis par info@fli.fr — une recette back-office en cours. Les
-- deux lignes ont été restaurées depuis audit_log.old_values, mais le compteur
-- « 1 stagiaire, 1 inscription » du dry-run ne permettait pas de faire la
-- différence entre un résidu de script et un jeu d'essai vivant.
--
-- Le dry-run renvoie désormais, sous la clé a_supprimer, une ligne par stagiaire
-- avec sa date de création, ses codes d'inscription et l'auteur de la création,
-- lu dans audit_log.user_id. La sélection et la suppression ne changent pas.
...
BEGIN
  ...
  IF n_storage > 0 THEN
    ...
    EXCEPTION WHEN insufficient_privilege THEN
      RAISE EXCEPTION '% fichier(s) ZZTEST dans le stockage : supprimez-les via l''API Storage puis relancez', n_storage
        USING ERRCODE = '42501';
    END;
    BEGIN
...
COMMENT ON FUNCTION public.cleanup_zztest_data(boolean) IS
  'Nettoyage ZZTEST. Le dry-run liste sous a_supprimer chaque stagiaire avec sa date de création, ses codes d''inscription et l''auteur de la création (audit_log.user_id), pour ne pas effacer une recette en cours.';
