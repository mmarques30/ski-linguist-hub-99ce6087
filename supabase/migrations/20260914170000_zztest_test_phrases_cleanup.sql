-- Kit ZZTEST : test_phrases (et tout référentiel marqué ZZTEST) entre dans le nettoyage.
-- Supprime les 8 phrases de démo C.3 (dont ZZTEST-CONCL-02 au tutoiement).
-- Réversible : les phrases métier reviendront avec l'import JSON du point 7.

INSERT INTO public.audit_log (action, table_name, new_values)
SELECT
  'zztest_test_phrases_cleanup',
  'test_phrases',
  jsonb_build_object(
    'codes', COALESCE(jsonb_agg(code ORDER BY code), '[]'::jsonb),
    'ids', COALESCE(jsonb_agg(id ORDER BY code), '[]'::jsonb)
  )
FROM public.test_phrases
WHERE code ILIKE 'ZZTEST%'
   OR text_fr ILIKE '%ZZTEST%';

DELETE FROM public.test_phrases
WHERE code ILIKE 'ZZTEST%'
   OR text_fr ILIKE '%ZZTEST%';
...
COMMENT ON FUNCTION public.cleanup_zztest_data(boolean) IS
  'Supprime les jeux ZZTEST / @example.invalid et les phrases test_phrases ZZTEST. dry_run=true par défaut. Admin uniquement.';
